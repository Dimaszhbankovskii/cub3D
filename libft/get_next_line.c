...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/18 16:59:03 by vjose             #+#    #+#             */
/*   Updated: 2022/09/18 16:59:04 by vjose            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_next_line(int fd, char **line)
{
	...
	if (fd < 0 || fd > 255 || BUFFER_SIZE < 1)
		return (-1);
	if (!ft_init(&buffer, &read_bytes))
		return (ft_free(-1, &buffer, NULL));
	while (!ft_findline(str) && read_bytes)
	{
		read_bytes = read(fd, buffer, BUFFER_SIZE);
		if (read_bytes < 0)
			return (ft_free(read_bytes, &buffer, &str));
		buffer[read_bytes] = '\0';
