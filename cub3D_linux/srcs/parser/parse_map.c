...
static void	define_position_hero(t_main *data, char dir, int row, int col)
{
...
void	check_map_borders(t_main *data)
{
	...
	while (++i < data->map.height)
	{
		...
		while (data->map.p[i][++j])
		{
			if (ft_strchr("0NSEW", data->map.p[i][j]))
			{
				if (i == 0 || (i == data->map.height - 1) || \
				j == 0 || (j == (int)ft_strlen(data->map.p[i]) - 1))
					end_program(data, ERROR_BORDERS_MAP, 1);
				if (data->map.pr[i - 1][j] == ' ' || data->map.pr[i + 1][j] == ' ' || \
				data->map.pr[i][j - 1] == ' ' || data->map.pr[i][j + 1] == ' ' || \
				data->map.pr[i - 1][j - 1] == ' ' || data->map.pr[i - 1][j + 1] == ' ' || \
				data->map.pr[i + 1][j - 1] == ' ' || data->map.pr[i + 1][j + 1] == ' ')
					end_program(data, ERROR_BORDERS_MAP, 1);
			}
		}
